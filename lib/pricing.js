...
export const TEAM_TIERS = [
  { minSeats: 2,  maxSeats: 5,  perSeatCents: 36000 },
  { minSeats: 6,  maxSeats: 10, perSeatCents: 28000 },
]
...
export function studentPrice(amountCents) {
  return Math.round(amountCents * (1 - STUDENT_DISCOUNT))
}

export function getTeamPerSeat(seats) {
  const tier = TEAM_TIERS.find(t => seats >= t.minSeats && seats <= t.maxSeats)
  return tier ? tier.perSeatCents : null
}
...
export function getPriceForTier(tier, seats = 1, examAddon = false) {
  ...
  if (tier === 'team') {
    const perSeat = getTeamPerSeat(seats) // null for 11+ seats
